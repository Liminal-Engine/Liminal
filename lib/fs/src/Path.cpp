...
namespace fs {
    class Path::_PathImpl {
        ...
        Status __createRegularFile(const _PathImpl &impl, const Permission &perms) const noexcept {
            std::ofstream tmp(impl.asStr());
            if ( !tmp.is_open() ) return Status::E_CREATE_FILE;
            tmp.close();
            impl.updatePermissions(perms);
            return Status::OK;
        };

        Status __createDirectory(const _PathImpl &impl, const Permission &perms) const noexcept {
            if ( !std::filesystem::create_directory(impl.asStr()) ) return Status::E_CREATE_DIR;
            impl.updatePermissions(perms);
            return Status::OK;
        }

        Status __createSymLink(const _PathImpl &impl, const Path &target, const Permission &perms) const noexcept {
            if (symlink(target.asStr().c_str(), impl.asStr().c_str()) == -1) {
                logger::error << "Error while creating symbolic link " << impl.asStr() << "to " << target.asStr();
                logger::error << ": " << std::string(strerror(errno)) << std::endl;
                return Status::E_CREATE_FILE;
            }
            impl.updatePermissions(perms);
            return Status::OK;
        }

        Status __createFifo(const _PathImpl &impl, const Permission &perms) const noexcept {
            if (mkfifo(impl.asStr().c_str(), perms.asModeT()) == -1) {
                logger::error << "Error while creating FIFO " << impl.asStr();
                logger::error << ": " << std::string(strerror(errno)) << std::endl;
                return Status::E_CREATE_FILE;
            }
            return Status::OK;
        ...
        Status create(
            ...
            try {
                switch (type) {
                    case Entry::Type::REGULAR_FILE: return this->__createRegularFile(tmpImpl, perms);
                    case Entry::Type::DIRECTORY: return this->__createDirectory(tmpImpl, perms);
                    case Entry::Type::SYM_LINK: return this->__createSymLink(tmpImpl, symLinkTarget, perms);
                    case Entry::Type::FIFO: return this->__createFifo(tmpImpl, perms);
                    case Entry::Type::SOCKET:
                    case Entry::Type::BLOCK_DEVICE:
                    case Entry::Type::CHARACTER_DEVICE: return Status::E_CREATE_FORBIDDEN_TYPE;
                    default:

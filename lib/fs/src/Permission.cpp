...
namespace fs {
    ...
    class Permission::_PImpl {
        ...
        _PImpl(const int &unixFormat) noexcept:
        _PImpl(static_cast<mode_t>(unixFormat))
        {}
        ...
        int asUnixFormat(void) const noexcept {
            ...
            if (this->_ownerCanWrite) res += 0200;
            if (this->_ownerCanExec) res += 0100;
            if (this->_groupCanRead) res += 040;
            if (this->_groupCanWrite) res += 020;
            if (this->_groupCanExec) res += 010;
            if (this->_otherCanRead) res += 04;
    ...
    Permission::Permission(const int &unixFormat) noexcept :
    _impl(std::make_unique<Permission::_PImpl>(unixFormat))
    {}
    ...
    Permission Permission::operator+(const Permission &other) const noexcept { return this->_impl->operator+(other); }
    Permission Permission::operator-(const Permission &other) const noexcept { return this->_impl->operator-(other); }

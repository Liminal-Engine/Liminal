...
namespace rhi {
    namespace __private {
        namespace __registry {
            ...
            const resource::Handle __Mesh::getHandle(const std::string &name) const { return this->__impl->getHandle(name); }
            const resource::Mesh *__Mesh::getResource(resource::Handle handle) const { return this->__impl->getResource(handle); }

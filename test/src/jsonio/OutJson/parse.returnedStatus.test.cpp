...
namespace test {
    namespace jsonio {
        namespace OutJson {
            ...
            using JsonIOOutJsonParseParam = std::pair<
                ::fs::Path, // path of file to parse
                ::jsonio::Status // expected Status
            >;

            class JsonIOOutJsonParseMethod : public ::testing::TestWithParam<JsonIOOutJsonParseParam> {

                protected:
                    ::jsonio::OutJson _instance;

                    JsonIOOutJsonParseMethod(void) :
                    _instance()
                    {}

            };

            TEST_P(JsonIOOutJsonParseMethod, ReturnsGivenStatus) {
                EXPECT_EQ(_instance.parse(GetParam().first), GetParam().second);
            };

            INSTANTIATE_TEST_SUITE_P(
                WhenGivenInvalidJsonContent,
                JsonIOOutJsonParseMethod,
                testing::Values(

...
class TimeDateToStrMethod : public ::testing::TestWithParam<TimeDateToStrParams> {
    ...
    protected:
        ...
        _instances{time_test::createInstances(std::get<0>(GetParam()), std::get<1>(GetParam()))}
        {}

};

TEST_P(TimeDateToStrMethod, ReturnsGivenString) {
    for (const time_::Date &date : _instances) {
        if (std::get<2>(GetParam()).has_value()) EXPECT_EQ(date.toStr(std::get<2>(GetParam()).value()), std::get<3>(GetParam()));
        else EXPECT_EQ(date.toStr(), std::get<3>(GetParam()));
    }
};
